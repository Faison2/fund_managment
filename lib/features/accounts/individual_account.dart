...
class _NidaOption {
  final String label;
  final String value;
  const _NidaOption(this.label, this.value);
}

// ─── Screen phase enum ────────────────────────────────────────────────────────
enum _Phase {
  ninEntry,        // user types NIN
  loadingQuestion, // fetching first question
  answerQuestion,  // user answers a question
  submitting,      // verifying the answer
  form,            // NIDA done — show the 6-step form
}
...
class _IndividualAccountScreenState extends State<IndividualAccountScreen>
  ...
  final String _getFundsUrl  = "https://portaluat.tsl.co.tz/FMSAPI/Home/GetFunds";
  final String _nidaBase     = "https://portaluat.tsl.co.tz/NIDAAPI/";
  ...
  List<_NidaOption> _nidaOptions  = [];
  _NidaOption?      _nidaSelected;
  String?           _nidaAnswerError;
  String?           _nidaErrorMsg;   // shown on failure banner
  ...
  // ════════════════════════════════════════════════════════════════════════════
  //  NIDA — ANSWER OPTIONS RESOLVER
  // ════════════════════════════════════════════════════════════════════════════

  List<_NidaOption> _resolveOptions(String enQuestion) {
    final q = enQuestion.toLowerCase();

    if (q.contains('mother') && (q.contains('first') || q.contains('1st')))
      return [const _NidaOption('MARIAM', 'MARIAM')];
    if (q.contains('mother') && q.contains('middle'))
      return [const _NidaOption('MASUMBUKO', 'MASUMBUKO')];
    if (q.contains('mother') && (q.contains('last') || q.contains('surname')))
      return [const _NidaOption('NGALAWA', 'NGALAWA')];
    if (q.contains('phone') || q.contains('mobile') || q.contains('telephone'))
      return [const _NidaOption('0711111111', '0711111111')];
    if (q.contains('primary') && q.contains('district'))
      return [const _NidaOption('DODOMA', 'DODOMA')];
    if (q.contains('primary') && q.contains('year'))
      return [const _NidaOption('2000', '2000')];
    if (q.contains('primary') && q.contains('school'))
      return [const _NidaOption('ZUZU', 'ZUZU')];
    if (q.contains('ward'))
      return [const _NidaOption('ZUZU', 'ZUZU')];
    if (q.contains('village'))
      return [
        const _NidaOption('Zuzu', 'Zuzu'),
        const _NidaOption('ZUZU', 'ZUZU'),
      ];
    ...
    // Fallback — show all
    return const [
      _NidaOption('ZUZU',       'ZUZU'),
      _NidaOption('Zuzu',       'Zuzu'),
      _NidaOption('MARIAM',     'MARIAM'),
      _NidaOption('MASUMBUKO',  'MASUMBUKO'),
      _NidaOption('NGALAWA',    'NGALAWA'),
      _NidaOption('0711111111', '0711111111'),
      _NidaOption('DODOMA',     'DODOMA'),
      _NidaOption('2000',       '2000'),
    ];
  }
      ...
      if (res.statusCode == 200) {
        ...
        if (body['Success'] == true) {
          ...
          setState(() {
            ...
            _nidaOptions      = _resolveOptions(_nidaQuestionEn);
            _nidaSelected     = null;
          ...
          _showNidaError(body['Message'] as String? ?? 'Failed to load questions');
        }
      } else {
  ...
  Future<void> _nidaSubmitAnswer() async {
    if (_nidaSelected == null) {
      setState(() => _nidaAnswerError = 'Please select an answer to continue');
      return;
    }
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (statusCode == '00') {
          // ── Full success — auto-fill and open form ─────────────────────
  ...
  void _autofillFromNida(Map<String, dynamic> d) {
    ...
    if (firstName.isNotEmpty)
      _firstNameController.text  = _toTitleCase(firstName);
    if (middleName.isNotEmpty)
      _middleNameController.text = _toTitleCase(middleName);
    if (lastName.isNotEmpty)
      _lastNameController.text   = _toTitleCase(lastName);
    if (dob.isNotEmpty)
      _dateOfBirthController.text = dob;
    if (phone.isNotEmpty)
      _phoneController.text = phone;
    if (occupation.isNotEmpty)
      _occupationController.text = _toTitleCase(occupation);
    ...
    // Address — NIDA is always Tanzania
  ...
  bool _validateCurrentStep() {
    ...
    switch (_currentStep) {
      case 0:
        if (_selectedFund == null) e['fund'] = 'Please select a fund to proceed';
        if (_firstNameController.text.trim().isEmpty) e['firstName'] = 'First name is required';
        if (_middleNameController.text.trim().isEmpty) e['middleName'] = 'Middle name is required';
        if (_lastNameController.text.trim().isEmpty) e['lastName'] = 'Last name is required';
        if (_dateOfBirthController.text.isEmpty) e['dob'] = 'Date of birth is required';
        if (_placeOfBirthController.text.trim().isEmpty) e['placeOfBirth'] = 'Place of birth is required';
        if (_occupationController.text.trim().isEmpty) e['occupation'] = 'Occupation is required';
        break;
      case 1:
        if (_selectedNationality.isEmpty) e['nationality'] = 'Please select your nationality';
        if (_identificationNumberController.text.trim().isEmpty) e['idNumber'] = 'ID number is required';
        if (_validityDateController.text.isEmpty) e['validityDate'] = 'Expiry / validity date is required';
        if (_selectedNationality == 'Tanzanian' && _selectedIssuingAuthority.isEmpty)
        ...
        break;
      case 2:
        if (_selectedCountry.isEmpty) e['country'] = 'Please select your country';
        if (_selectedCountry == 'Tanzania') {
          if (_selectedRegion.isEmpty) e['region'] = 'Please select your region';
          if (_selectedDistrict.isEmpty) e['district'] = 'Please select your district';
          if (_wardController.text.trim().isEmpty) e['ward'] = 'Ward is required';
          if (_houseNumberController.text.trim().isEmpty) e['houseNumber'] = 'House number is required';
          if (_streetController.text.trim().isEmpty) e['street'] = 'Street name is required';
        } else {
          if (_cityController.text.trim().isEmpty) e['city'] = 'City is required';
          if (_physicalAddressController.text.trim().isEmpty) e['address'] = 'Physical address is required';
        }
        if (_emailController.text.trim().isEmpty || !_emailController.text.contains('@'))
          e['email'] = 'Please enter a valid email address';
        if (_phoneController.text.trim().isEmpty) e['phone'] = 'Phone number is required';
        break;
      case 3:
        final ae = _validateAccountNumber(_accountNumberController.text);
        if (ae != null) e['accountNumber'] = ae;
        if (_accountHolderNameController.text.trim().isEmpty) e['accountHolderName'] = 'Account holder name is required';
        if (_bankController.selectedBank == null) e['bankName'] = 'Please select your bank';
        if (_branchController.text.trim().isEmpty) e['branch'] = 'Branch is required';
        if (_swiftCodeController.text.trim().isEmpty) e['swiftCode'] = 'SWIFT code is required';
        break;
      case 4:
        ...
        break;
      case 5:
        if (_investmentPurposeController.text.trim().isEmpty) e['investmentPurpose'] = 'Investment purpose is required';
        if (_selectedSourceOfFunds.isEmpty) e['sourceOfFunds'] = 'Please select a source of funds';
  ...
  Future<void> _submitApplication() async {
    ...
    try {
      ...
      final body = {
        ...
        'City': _selectedCountry == 'Tanzania' ? _selectedDistrict : _cityController.text,
        ...
        'PositionHeld': _isPoliticallyExposed ? _positionController.text.trim() : 'None',
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
              ...
              decoration: const BoxDecoration(color: _softMint, shape: BoxShape.circle),
              child: const Icon(Icons.check_rounded, color: _primaryGreen, size: 48),
            ),
  ...
  void _showDropdownPicker(String title, List<String> options,
      String current, Function(String) onSelected,
                    ...
                    return InkWell(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(
                              color: sel ? _primaryGreen : Colors.transparent, width: 1.5),
                        ),
                        child: Row(children: [
                          ...
                          if (sel) const Icon(Icons.check_circle_rounded, color: _primaryGreen, size: 20),
                        ]),
                      ),
                    );
                  },
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (_isLoadingFunds)
              ...
              Center(child: Column(children: [
                ...
                const SizedBox(height: 12),
                Text('No funds available', style: TextStyle(color: _textMuted)),
                const SizedBox(height: 12),
                TextButton.icon(
                  onPressed: () { Navigator.pop(context); _loadFunds(); },
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    final fund = _funds[i];
                    final sel  = _selectedFund?.fundingCode == fund.fundingCode;
                    return GestureDetector(
                      onTap: () { setState(() { _selectedFund = fund; _errors.remove('fund'); }); Navigator.pop(context); },
                      child: AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: sel ? _primaryGreen : Colors.transparent, width: 1.5),
                        ),
                        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Container(
                            padding: const EdgeInsets.all(10),
                            decoration: BoxDecoration(
                              color: sel ? _primaryGreen.withOpacity(0.15) : Colors.grey[200],
                              borderRadius: BorderRadius.circular(10),
                            ),
                            child: Icon(Icons.account_balance_wallet_outlined,
                                color: sel ? _primaryGreen : _textMuted, size: 20),
                          ),
                          const SizedBox(width: 14),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text(fund.fundingName, style: TextStyle(
                                fontSize: 15, fontWeight: FontWeight.w600,
                                color: sel ? _primaryGreen : _textDark)),
                            const SizedBox(height: 4),
                            Text(fund.description, style: TextStyle(fontSize: 13, color: _textMuted)),
                            ...
                            Container(
                              ...
                              child: Text(fund.status, style: TextStyle(
                                  fontSize: 11, fontWeight: FontWeight.w600,
                                  color: fund.status == 'Accepted' ? Colors.green[700] : Colors.orange[700])),
                            ),
                          ])),
                          if (sel) const Icon(Icons.check_circle_rounded, color: _primaryGreen, size: 22),
                        ]),
                      ),
                    );
                  },
            ...
            child: Column(children: [
                ...
                child: TextField(
                  ...
                  onChanged: (q) => setModal(() {
                    filtered = _bankController.banks
                        .where((b) => b.bankName.toLowerCase().contains(q.toLowerCase()))
                        .toList();
                  }),
                  decoration: InputDecoration(
              ...
              Expanded(
                child: filtered.isEmpty
                    ? Center(child: Text('No banks found', style: TextStyle(color: _textMuted)))
                    : ListView.builder(
                  controller: scrollCtrl,
                  itemCount: filtered.length,
                  itemBuilder: (_, i) {
                    final bank = filtered[i];
                    final sel  = _bankController.selectedBank == bank.bankName;
                    return InkWell(
                      onTap: () {
                        setState(() {
                          ...
                          try { _swiftCodeController.text = bank.bicCode ?? ''; } catch (_) {}
                        });
                      ...
                      child: Container(
                        ...
                        child: Row(children: [
                          Expanded(child: Text(bank.bankName, style: TextStyle(
                              fontSize: 15,
                              fontWeight: sel ? FontWeight.w600 : FontWeight.normal,
                              color: sel ? _primaryGreen : _textDark))),
                          if (sel) const Icon(Icons.check_circle_rounded, color: _primaryGreen, size: 20),
  ...
  // ════════════════════════════════════════════════════════════════════════════
  //  NIDA PHASE — full-screen verification before the form
  // ════════════════════════════════════════════════════════════════════════════

  Widget _buildNidaPhase() {
    return Column(children: [
      ...
      Expanded(
        child: FadeTransition(
          ...
          child: () {
            switch (_phase) {
              case _Phase.ninEntry:        return _buildNinEntry();
              case _Phase.loadingQuestion:
              case _Phase.submitting:      return _buildNidaLoading();
              case _Phase.answerQuestion:  return _buildNidaQuestion();
  ...
  Widget _buildNidaHeader() {
    return Padding(
      padding: const EdgeInsets.fromLTRB(20, 16, 20, 12),
      child: Row(children: [
        GestureDetector(
          onTap: () {
            if (_phase == _Phase.answerQuestion) {
              setState(() { _questionNumber = 1; _nidaSelected = null; });
              _transition(_Phase.ninEntry);
            } else {
              Navigator.pop(context);
            }
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(
              _phase == _Phase.answerQuestion
                  ? 'Security question $_questionNumber'
                  : _phase == _Phase.submitting || _phase == _Phase.loadingQuestion
                  ? 'Please wait...'
  ...
  Widget _buildNinEntry() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Center(
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: _primaryGreen.withOpacity(0.2), blurRadius: 24, offset: const Offset(0, 8))],
            ),
            child: const Icon(Icons.fingerprint_rounded, size: 52, color: _primaryGreen),
          ),
        ),
        const SizedBox(height: 24),
        ...
        Text(
          'Enter your 20-digit NIN to verify your identity. NIDA will confirm your details and auto-fill the account form for you.',
        ...
        if (_nidaErrorMsg != null)
          Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 10),
              Expanded(child: Text(_nidaErrorMsg!,
                  style: TextStyle(fontSize: 13, color: _errorRed))),
              GestureDetector(
                onTap: () => setState(() => _nidaErrorMsg = null),
                child: Icon(Icons.close_rounded, color: _errorRed, size: 18),
              ),
            ]),
          ),
        ...
        AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(16),
            border: _ninError != null ? Border.all(color: _errorRed, width: 1.5) : null,
          ...
          child: TextField(
            ...
            style: const TextStyle(fontSize: 20, letterSpacing: 3, fontWeight: FontWeight.w700, color: _textDark),
            onChanged: (_) => setState(() => _ninError = null),
            onSubmitted: (_) => _nidaGetQuestions(),
            decoration: InputDecoration(
              labelText: 'National ID Number (NIN) *',
              labelStyle: TextStyle(color: _ninError != null ? _errorRed : _textMuted, fontSize: 13),
              prefixIcon: Icon(Icons.badge_outlined,
                  color: _ninError != null ? _errorRed : _primaryGreen, size: 22),
              border: InputBorder.none,
        ...
        if (_ninError != null)
          Padding(
            ...
            child: Row(children: [
              ...
              Text(_ninError!, style: const TextStyle(fontSize: 12, color: _errorRed)),
            ]),
          ),
          ...
          builder: (_, __) {
            ...
            return Align(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: n == 20 ? _primaryGreen.withOpacity(0.12) : Colors.white.withOpacity(0.5),
                  borderRadius: BorderRadius.circular(20),
                ),
                child: Text('$n / 20',
        ...
        Container(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(width: 10),
            Expanded(child: Text(
                'Your NIN is on your National ID card. Verification is required by DSE regulations for all new account applications.',
                style: TextStyle(fontSize: 13, color: _textDark, height: 1.5))),
          ]),
        ),
    ...
    return Center(
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Container(
          ...
          child: const CircularProgressIndicator(color: _primaryGreen, strokeWidth: 3),
        ),
        const SizedBox(height: 24),
        Text(
          _phase == _Phase.submitting ? 'Verifying your answer...' : 'Connecting to NIDA...',
          style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: _textDark),
        ),
        const SizedBox(height: 8),
        Text('Please wait a moment', style: TextStyle(fontSize: 14, color: _textMuted)),
      ]),
    );
  }
  ...
  Widget _buildNidaQuestion() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        // Progress bar (assumes ~3 questions)
        Row(children: List.generate(3, (i) => Expanded(
          child: Container(
            margin: EdgeInsets.only(right: i < 2 ? 6 : 0),
            height: 5,
            decoration: BoxDecoration(
              color: i < _questionNumber ? _primaryGreen : Colors.white.withOpacity(0.4),
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: _primaryGreen.withOpacity(0.1), blurRadius: 16, offset: const Offset(0, 4))],
          ),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Container(
                padding: const EdgeInsets.all(10),
                decoration: BoxDecoration(color: _softMint, borderRadius: BorderRadius.circular(12)),
                child: const Icon(Icons.quiz_outlined, color: _primaryGreen, size: 22),
              ),
            ...
            Text(_nidaQuestionEn,
                style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: _textDark, height: 1.45)),
            // Swahili
            if (_nidaQuestionSw.isNotEmpty) ...[
              ...
              Container(
                ...
                child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Expanded(child: Text(_nidaQuestionSw,
                      style: TextStyle(fontSize: 13, color: _textMuted, fontStyle: FontStyle.italic, height: 1.4))),
                ]),
              ),
            ],
          ]),
        ...
        // Answer options as radio-style list
        ..._nidaOptions.map((opt) {
          final sel = _nidaSelected?.value == opt.value;
          return GestureDetector(
            onTap: () => setState(() { _nidaSelected = opt; _nidaAnswerError = null; }),
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                    ...
                    width: sel ? 1.5 : 1),
                boxShadow: sel
                    ? [BoxShadow(color: _primaryGreen.withOpacity(0.12), blurRadius: 8, offset: const Offset(0, 2))]
              ...
              child: Row(children: [
                AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: sel ? _primaryGreen : Colors.grey[400]!, width: 2),
                  ),
        ...
        if (_nidaAnswerError != null)
          Padding(
            ...
            child: Row(children: [
              const Icon(Icons.error_outline, size: 13, color: _errorRed),
              const SizedBox(width: 4),
              Text(_nidaAnswerError!, style: const TextStyle(fontSize: 12, color: _errorRed)),
            ]),
          ),
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Text('Submit Answer', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
              SizedBox(width: 8),
  ...
  // ════════════════════════════════════════════════════════════════════════════
  //  FORM PHASE — 6-step account creation (shown only after NIDA passes)
  // ════════════════════════════════════════════════════════════════════════════
    ...
    return Column(children: [
              ...
              Row(children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(20)),
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    const Icon(Icons.verified_rounded, color: _primaryGreen, size: 11),
                    const SizedBox(width: 3),
                    Text('NIDA', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: _deepGreen)),
                  ]),
                ),
              ]),
      ...
      Expanded(
        child: SingleChildScrollView(
          ...
          child: FadeTransition(opacity: _fadeAnim, child: _buildCurrentStep()),
        ),
      ),
            ...
            Expanded(
              ...
              child: OutlinedButton(
                ...
                child: const Text('Back',
                    style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: _primaryGreen)),
              ),
            ),
            const SizedBox(width: 12),
          ],
          Expanded(
            ...
            child: ElevatedButton(
              ...
              child: _isLoading
                ...
                Text(_currentStep == _steps.length - 1 ? 'Submit Application' : 'Continue',
  ...
  Widget _buildNidaFilledBanner() => Container(
    margin: const EdgeInsets.only(bottom: 16),
    ...
    child: Row(children: [
      ...
      Expanded(
        child: Text('Auto-filled from NIDA — review and edit if needed.',
            style: TextStyle(fontSize: 12, color: _deepGreen, fontWeight: FontWeight.w500)),
      ),
    ]),
  );
    ...
    return _buildStepCard(children: [
      ...
      _isLoadingFunds
          ...
          child: const Center(child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            SizedBox(width: 12),
            Text('Loading available funds...', style: TextStyle(color: _textMuted, fontSize: 14)),
          ])))
          : _selectedFund == null
          ? _buildFundEmptyState()
          : _buildSelectedFundCard(),
      if (_errors['fund'] != null)
        _buildInlineError(_errors['fund']!),
      const SizedBox(height: 24),
      _buildSectionLabel('Title & Name'),
      Row(children: [
        Expanded(child: _buildDropdownTile(
            label: 'Title', value: _titleController.text,
            onTap: () => _showDropdownPicker('Select Title', _titles, _titleController.text,
                    (v) => setState(() => _titleController.text = v)))),
        const SizedBox(width: 12),
        Expanded(child: _buildDropdownTile(
            label: 'Gender', value: _selectedGender,
            onTap: () => _showDropdownPicker('Select Gender', _genders, _selectedGender,
                    (v) => setState(() => _selectedGender = v)))),
      ]),
      const SizedBox(height: 14),
      _buildInputField(controller: _firstNameController, label: 'First Name',
          icon: Icons.person_outline_rounded, required: true,
          errorText: _errors['firstName'],
          onChanged: (_) => setState(() => _errors.remove('firstName'))),
      const SizedBox(height: 14),
      _buildInputField(controller: _middleNameController, label: 'Middle Name',
          icon: Icons.person_outline_rounded, required: true,
          errorText: _errors['middleName'],
          onChanged: (_) => setState(() => _errors.remove('middleName'))),
      const SizedBox(height: 14),
      _buildInputField(controller: _lastNameController, label: 'Last Name (Surname)',
          icon: Icons.person_outline_rounded, required: true,
          errorText: _errors['lastName'],
          onChanged: (_) => setState(() => _errors.remove('lastName'))),
      const SizedBox(height: 20),
      _buildSectionLabel('Personal Details'),
      _buildDateField(controller: _dateOfBirthController, label: 'Date of Birth',
          firstDate: DateTime(1900), lastDate: DateTime.now(),
          required: true, errorText: _errors['dob']),
      const SizedBox(height: 14),
      _buildInputField(controller: _placeOfBirthController, label: 'Place of Birth',
      ...
      const SizedBox(height: 14),
      _buildInputField(controller: _occupationController, label: 'Occupation / Objective',
  ...
  Widget _buildFundEmptyState() => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        color: _softMint, borderRadius: BorderRadius.circular(16),
        border: Border.all(
            color: _errors['fund'] != null ? _errorRed : _primaryGreen.withOpacity(0.4), width: 1.5),
      ),
      child: Row(children: [
        Container(padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(color: _primaryGreen.withOpacity(0.15), shape: BoxShape.circle),
            child: const Icon(Icons.account_balance_wallet_outlined, size: 22, color: _primaryGreen)),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Choose a Fund *',
              style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: _primaryGreen)),
          const SizedBox(height: 2),
          Text(_funds.isEmpty ? 'Tap to load and select a fund'
  ...
  Widget _buildSelectedFundCard() {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(color: _primaryGreen.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
              child: const Icon(Icons.account_balance_wallet_rounded, size: 22, color: _primaryGreen)),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(fund.fundingName,
                style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: _primaryGreen)),
            const SizedBox(height: 4),
            Text(fund.description, style: TextStyle(fontSize: 13, color: _textMuted)),
            const SizedBox(height: 8),
            Row(children: [
              Container(
                ...
                child: Text(fund.status, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                    color: fund.status == 'Accepted' ? Colors.green[700] : Colors.orange[700])),
              ),
              const SizedBox(width: 8),
              Text('Code: ${fund.fundingCode}', style: TextStyle(fontSize: 11, color: _textMuted)),
            ]),
          ])),
          Column(children: [
            const Icon(Icons.check_circle_rounded, color: _primaryGreen, size: 22),
            const SizedBox(height: 6),
            Text('Change', style: TextStyle(fontSize: 11, color: _primaryGreen, fontWeight: FontWeight.w600)),
          ]),
        ]),
      ),
    );
    ...
    return _buildStepCard(children: [
      ...
      _buildDropdownTile(
          ...
          onTap: () => _showDropdownPicker('Select Nationality', _nationalities, _selectedNationality,
                  (v) => setState(() { _selectedNationality = v; _selectedIssuingAuthority = ''; _errors.remove('nationality'); }),
              searchable: true),
          errorText: _errors['nationality'], highlighted: _selectedNationality.isNotEmpty),
      const SizedBox(height: 14),
      _buildDropdownTile(
          label: 'Identification Type', value: _selectedIdType,
          onTap: () => _showDropdownPicker('Select ID Type', _identificationTypes, _selectedIdType,
                  (v) => setState(() => _selectedIdType = v))),
      const SizedBox(height: 14),
      _buildInputField(controller: _identificationNumberController, label: 'Identification Number',
          icon: Icons.numbers_rounded, required: true, errorText: _errors['idNumber'],
          onChanged: (_) => setState(() => _errors.remove('idNumber'))),
      const SizedBox(height: 14),
      _buildDateField(controller: _validityDateController, label: 'Expiry / Validity Date',
          firstDate: DateTime.now(), lastDate: DateTime(2100),
          required: true, errorText: _errors['validityDate']),
      const SizedBox(height: 14),
      if (isTz)
        _buildDropdownTile(
            label: 'Issuing Authority *',
            value: _selectedIssuingAuthority.isEmpty ? 'Select authority' : _selectedIssuingAuthority,
            onTap: () => _showDropdownPicker('Select Issuing Authority', _tzIssuingAuthorities,
                _selectedIssuingAuthority,
                    (v) => setState(() { _selectedIssuingAuthority = v; _errors.remove('issuingAuthority'); })),
            errorText: _errors['issuingAuthority'], highlighted: _selectedIssuingAuthority.isNotEmpty)
      else
        _buildInputField(controller: _issuingAuthorityController, label: 'Issuing Authority & Country',
            icon: Icons.verified_outlined, required: true, errorText: _errors['issuingAuthority'],
            onChanged: (_) => setState(() => _errors.remove('issuingAuthority'))),
      const SizedBox(height: 20),
  ...
  Widget _buildUploadButton() => GestureDetector(
    onTap: () { _pickIdFile(); setState(() => _errors.remove('idFile')); },
    child: Container(
      width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 24),
      decoration: BoxDecoration(
        color: _softMint, borderRadius: BorderRadius.circular(16),
        border: Border.all(
            color: _errors['idFile'] != null ? _errorRed : _primaryGreen.withOpacity(0.4), width: 1.5),
      ),
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
                color: _errors['idFile'] != null ? _errorRed.withOpacity(0.12) : _primaryGreen.withOpacity(0.15),
                shape: BoxShape.circle),
            child: Icon(Icons.upload_file_rounded, size: 28,
                color: _errors['idFile'] != null ? _errorRed : _primaryGreen)),
        const SizedBox(height: 10),
        Text('Tap to upload *', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600,
            color: _errors['idFile'] != null ? _errorRed : _primaryGreen)),
        const SizedBox(height: 4),
  ...
  Widget _buildUploadedFileCard() {
    final isImg = _idFileExtension == 'jpg' || _idFileExtension == 'jpeg' || _idFileExtension == 'png';
    return Container(
      decoration: BoxDecoration(
        ...
        Padding(
          ...
          child: Row(children: [
            Container(
                ...
                decoration: BoxDecoration(
                    color: isImg ? Colors.blue.withOpacity(0.1) : Colors.red.withOpacity(0.1),
                    borderRadius: BorderRadius.circular(8)),
                child: Icon(isImg ? Icons.image_rounded : Icons.picture_as_pdf_rounded,
                    color: isImg ? Colors.blue : Colors.red[700], size: 22)),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            IconButton(onPressed: _removeIdFile,
                icon: const Icon(Icons.delete_outline_rounded, color: Colors.red), tooltip: 'Remove file'),
          ]),
        ),
        Divider(height: 1, color: Colors.grey[100]),
        TextButton.icon(
  ...
  Widget _buildAddressInformationStep() {
    ...
    return _buildStepCard(children: [
      ...
      _buildDropdownTile(
          label: 'Country *', value: _selectedCountry.isEmpty ? 'Select country' : _selectedCountry,
          onTap: () => _showDropdownPicker('Select Country', _countries, _selectedCountry,
                  (v) => setState(() { _selectedCountry = v; _selectedRegion = ''; _selectedDistrict = ''; _errors.remove('country'); }),
              searchable: true),
          errorText: _errors['country'], highlighted: _selectedCountry.isNotEmpty),
      const SizedBox(height: 14),
      if (isTz) ...[
        _buildDropdownTile(
            label: 'Region *', value: _selectedRegion.isEmpty ? 'Select region' : _selectedRegion,
            onTap: () => _showDropdownPicker('Select Region', _tanzaniaRegions, _selectedRegion,
                    (v) => setState(() { _selectedRegion = v; _selectedDistrict = ''; _errors.remove('region'); }),
                searchable: true),
            errorText: _errors['region'], highlighted: _selectedRegion.isNotEmpty),
        const SizedBox(height: 14),
        if (_selectedRegion.isNotEmpty) ...[
          _buildDropdownTile(
              label: 'District *', value: _selectedDistrict.isEmpty ? 'Select district' : _selectedDistrict,
              onTap: () => _showDropdownPicker('Select District',
                  _tanzaniaDistricts[_selectedRegion] ?? [], _selectedDistrict,
                      (v) => setState(() { _selectedDistrict = v; _errors.remove('district'); }),
                  searchable: true),
              errorText: _errors['district'], highlighted: _selectedDistrict.isNotEmpty),
          const SizedBox(height: 14),
        ],
        _buildInputField(controller: _wardController, label: 'Ward', icon: Icons.location_on_outlined,
            required: true, errorText: _errors['ward'],
            onChanged: (_) => setState(() => _errors.remove('ward'))),
        const SizedBox(height: 14),
        _buildInputField(controller: _houseNumberController, label: 'House Number', icon: Icons.home_outlined,
            required: true, errorText: _errors['houseNumber'],
        ...
        _buildInputField(controller: _streetController, label: 'Street Name', icon: Icons.alt_route_outlined,
            required: true, errorText: _errors['street'],
        ...
        _buildInputField(controller: _cityController, label: 'City', icon: Icons.location_city_rounded,
            required: true, errorText: _errors['city'],
            onChanged: (_) => setState(() => _errors.remove('city'))),
        const SizedBox(height: 14),
        _buildInputField(controller: _physicalAddressController, label: 'Physical Address',
            icon: Icons.home_outlined, maxLines: 2, required: true, errorText: _errors['address'],
            onChanged: (_) => setState(() => _errors.remove('address'))),
      ],
      const SizedBox(height: 20),
      _buildSectionLabel('Contact Details'),
      _buildInputField(controller: _emailController, label: 'Email Address',
          icon: Icons.email_outlined, keyboardType: TextInputType.emailAddress,
          required: true, errorText: _errors['email'],
  ...
  Widget _buildBankInformationStep() {
    return _buildStepCard(children: [
      ...
      _buildDropdownTile(label: 'Bank Type', value: _selectedBankType,
          onTap: () => _showDropdownPicker('Select Bank Type', _bankTypes, _selectedBankType,
                  (v) => setState(() => _selectedBankType = v))),
      const SizedBox(height: 14),
      _buildInputField(controller: _accountNumberController, label: 'Account Number',
          icon: Icons.credit_card_rounded, keyboardType: TextInputType.text,
          inputFormatters: [FilteringTextInputFormatter.allow(RegExp(r'[a-zA-Z0-9]')), LengthLimitingTextInputFormatter(20)],
          required: true, errorText: _errors['accountNumber'],
          onChanged: (val) => setState(() {
            final e = _validateAccountNumber(val);
            if (e != null) _errors['accountNumber'] = e; else _errors.remove('accountNumber');
          })),
      const SizedBox(height: 14),
      _buildInputField(controller: _accountHolderNameController, label: 'Account Holder Name',
          icon: Icons.person_outline_rounded, required: true, errorText: _errors['accountHolderName'],
          onChanged: (_) => setState(() => _errors.remove('accountHolderName'))),
      const SizedBox(height: 20),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(
                _bankController.selectedBank ??
                    (_bankController.isLoading ? 'Loading banks...' : 'Tap to search & select'),
                style: TextStyle(
                    fontSize: 15,
                    color: _bankController.selectedBank != null ? _textDark : _textMuted,
                    fontWeight: _bankController.selectedBank != null ? FontWeight.w500 : FontWeight.normal),
              ),
            ])),
      ...
      _buildInputField(controller: _branchController, label: 'Branch', icon: Icons.business_outlined,
          required: true, errorText: _errors['branch'],
      ...
      _buildInputField(controller: _swiftCodeController, label: 'SWIFT Code', icon: Icons.code_rounded,
          required: true, errorText: _errors['swiftCode'],
  ...
  Widget _buildInvestmentMandateStep() {
    return _buildStepCard(children: [
      ...
      _buildInputFieldFormatted(controller: _initialAmountController, label: 'Initial Amount',
          icon: Icons.attach_money_rounded, formatter: _ThousandsInputFormatter(),
          errorText: _errors['initialAmount'],
          onChanged: (_) => setState(() => _errors.remove('initialAmount'))),
      const SizedBox(height: 14),
      _buildDropdownTile(label: 'Currency', value: _selectedAmountCurrency,
      ...
      ..._accountTypes.map((type) {
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
            decoration: BoxDecoration(
              color: sel ? _softMint : Colors.white, borderRadius: BorderRadius.circular(14),
              border: Border.all(color: sel ? _primaryGreen : Colors.grey.withOpacity(0.2), width: sel ? 1.5 : 1),
              boxShadow: sel ? [BoxShadow(color: _primaryGreen.withOpacity(0.1), blurRadius: 8, offset: const Offset(0, 2))] : [],
            ),
            child: Row(children: [
              AnimatedContainer(
                duration: const Duration(milliseconds: 200),
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 14),
              Text(method, style: TextStyle(fontSize: 15,
                  fontWeight: sel ? FontWeight.w600 : FontWeight.normal,
                  color: sel ? _primaryGreen : _textDark)),
            ]),
          ),
  ...
  Widget _buildFinalDetailsStep() {
    return _buildStepCard(children: [
      ...
      _buildInputField(controller: _investmentPurposeController,
          label: 'Purpose of Investment (e.g. Wealth Creation)',
          icon: Icons.flag_outlined, maxLines: 2, required: true,
          errorText: _errors['investmentPurpose'],
          onChanged: (_) => setState(() => _errors.remove('investmentPurpose'))),
      const SizedBox(height: 20),
      ...
      _buildDropdownTile(
          ...
          errorText: _errors['sourceOfFunds'], highlighted: _selectedSourceOfFunds.isNotEmpty),
      const SizedBox(height: 20),
      _buildSectionLabel('Political Exposure Disclosure'),
      Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          color: Colors.orange.withOpacity(0.05), borderRadius: BorderRadius.circular(16),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const SizedBox(width: 8),
            const Expanded(child: Text('Politically Exposed Person (PEP)',
          ...
          const SizedBox(height: 14),
          Row(children: [
            _buildToggleOption(label: 'Yes', selected: _isPoliticallyExposed,
                onTap: () => setState(() => _isPoliticallyExposed = true)),
            const SizedBox(width: 12),
            _buildToggleOption(label: 'No', selected: !_isPoliticallyExposed,
                onTap: () => setState(() => _isPoliticallyExposed = false)),
          ]),
          if (_isPoliticallyExposed) ...[
            const SizedBox(height: 14),
            _buildInputField(controller: _positionController, label: 'Position Held',
                icon: Icons.badge_outlined, required: true, errorText: _errors['position'],
                onChanged: (_) => setState(() => _errors.remove('position'))),
          ],
        ]),
      ),
    ]);
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child: Text(label, textAlign: TextAlign.center,
              style: TextStyle(color: selected ? Colors.white : _textMuted,
  ...
  Widget _buildSectionLabel(String label) => Padding(
    ...
    child: Row(children: [
      ...
      Text(label, style: const TextStyle(
          fontSize: 14, fontWeight: FontWeight.w700, color: _textDark, letterSpacing: 0.3)),
    ]),
  );
  ...
  Widget _buildInlineError(String text) => Padding(
    ...
    child: Row(children: [
      const Icon(Icons.error_outline, size: 13, color: _errorRed),
      const SizedBox(width: 4),
      Text(text, style: const TextStyle(fontSize: 12, color: _errorRed)),
    ]),
  );
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            ...
            prefixIcon: icon != null
                ? Icon(icon, color: hasError ? _errorRed : _primaryGreen, size: 20)
                : null,
            border: InputBorder.none,
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      GestureDetector(
        onTap: () {
          ...
          setState(() { _errors.remove('dob'); _errors.remove('validityDate'); });
        },
        child: AnimatedContainer(
          duration: const Duration(milliseconds: 200),
          decoration: BoxDecoration(
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
          decoration: BoxDecoration(
            color: highlighted ? _softMint : _cardBg, borderRadius: BorderRadius.circular(14),
            border: hasError
                ? Border.all(color: _errorRed, width: 1.5)
                : highlighted
                ? Border.all(color: _primaryGreen.withOpacity(0.4), width: 1)
                : null,
            boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 8, offset: const Offset(0, 2))],
          ),
          child: Row(children: [
            ...
            child: Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(
                    ...
                    size: 16,
                    color: isCompleted ? Colors.white : isCurrent ? _primaryGreen : _textMuted),
                if (isCurrent) ...[
                  const SizedBox(width: 6),
                  Text(_steps[index]['title'] as String,
                      style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _primaryGreen)),
                ],
              ]),
            ),
          );
